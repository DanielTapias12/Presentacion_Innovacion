...
export const Logo: React.FC<LogoProps> = ({ 
  ...
  // Use React's useId to generate unique IDs for gradients/filters
  // This prevents conflicts when multiple Logo components are rendered
  const idRaw = useId();
  const idClean = idRaw.replace(/:/g, ''); // Ensure safe ID string for SVG
    ...
    <div className={`flex ${layout === 'vertical' ? 'flex-col justify-center' : 'flex-row'} items-center gap-4 ${className}`}>
      ...
      <div className={`${iconSize} relative flex-shrink-0`}>
        ...
        >
          ...
          {/* Background Glow - Stronger opacity */}
          ...
          {/* Puzzle Piece Cutout Hint (Inclusion) - Clearer */}
